# lipRH 🚀

## 📌 Sobre o Projeto

O **lipRH** é uma plataforma digital inteligente desenvolvida para modernizar e centralizar os processos de Recursos Humanos dentro das empresas.

O projeto foi criado durante o **Hackathon Diadema**, com o objetivo de solucionar problemas relacionados à burocracia, comunicação interna e descentralização de informações no setor de RH.

A proposta do aplicativo é aproximar colaboradores e empresa através da tecnologia, oferecendo praticidade, automação e uma experiência mais eficiente para todos os usuários.

---

# ✨ Funcionalidades

* 📱 Interface moderna e intuitiva
* 💬 Chatbot inteligente integrado
* 🤖 Integração com IA Gemini
* 📄 Consulta de benefícios e informações
* 🗓️ Solicitação de férias e licenças
* 🔔 Notificações e comunicação interna

---

# 🛠️ Tecnologias Utilizadas

## Front-end / Mobile

* Kotlin
* Android Studio
* XML

## Back-end

* MySQL
* Integração com Gemini AI

## Ferramentas

* Figma
* GitHub

---

# 🤖 Inteligência Artificial

O lipRH conta com integração da **Gemini AI**, utilizada para oferecer:

* respostas rápidas aos colaboradores;
* suporte automatizado;
* auxílio em dúvidas frequentes;
* melhoria na experiência do usuário.

---

# 🎯 Objetivo

Nosso objetivo é transformar a experiência do RH corporativo através de uma solução moderna, escalável e inteligente, reduzindo processos manuais e aumentando a eficiência da comunicação entre empresa e colaboradores.

---

# 🌟 Diferenciais

* Centralização dos serviços de RH
* Automação de processos
* Interface acessível e intuitiva
* Experiência mobile completa
* Uso de inteligência artificial
* Escalabilidade para futuras integrações

---

# 👥 Equipe

Projeto desenvolvido para o **Hackathon Diadema** por:

* Thais Ferreira de Moura
* Pedro Henrique Guerdis Silva
* Ryan Ruiz Gonçalves
* Ester Amancio Abreu
* José Claudemir Galazine
* Miguel Vulcanis Neves
---

# 💡 Slogan

### *“Inovação que aproxima. O futuro do RH começa aqui.”*
